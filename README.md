# hOI! im tEMMIE!! and dis my friend... tEMMIE!!
<img src="temmie.gif" alt="temmie" height="330" width="600"/> <br>

## About Me
henlo! chronically online since 1984  
b.tech student in my final year
majoring in CyberSecurity, minoring in DevOps and making god knows what in Unity  
I do many qol and automation projects but never tend to upload them because honestly, they dont feel up to the mark for me.  
I hate how lazy AI has made me so I only keep work of the best quality with me, rest all is temporary.  
the voices are getting louder, they demand sacrifice...

<!-- Discord Presence -->
<a href="https://discord.com/users/94490510688792576">
  <img src="https://lanyard.cnrad.dev/api/692418341993709600?theme=dark&animatedDecoration=true&hideDiscrim=false&showDisplayName=true&borderRadius=20px" alt="Discord Presence" style="max-height: 200px; border-radius: 20px"/>
</a>
<!-- Steam Presence -->
<a href="https://steamcommunity.com/id/saddexed">
  <img src="https://steam-widget.com/widget/img?id=76561199469800966&gameList=TOP_GAMES_RECENT&gameListSize=3" alt="Steam Presence" style="max-height: 200px; border-radius: 20px"/>
</a>

## technologia!
[![My Skills](https://skillicons.dev/icons?i=aws,bash,cs,cpp,cloudflare,css,discord,discordjs,docker,dotnet,git,github,githubactions,gitlab,html,js,jenkins,kali,md,netlify,nginx,nodejs,npm,ps,pr,py,regex,selenium,terraform,ts,ubuntu,unity,vercel,vscode,windows,&theme=dark)](https://skillicons.dev)


## tEMMIE has goals... 
- [ ] Actually make meaningful contributions to opensource
- [ ] Master .NET
- [ ] Learn DSA properly (My ass is not learning this shi)
- [x] Make a portfolio website
- [ ] Make a personal website/showcase (hosted and everything)

## Projects worth showcasing (there are none)

- [llogin](https://github.com/saddexed/llogin) - Stupid university with its stupid captive portal that doesnt stupid work
- [weather-app](https://github.com/saddexed/Weather-App) - If you've never built one, are you even really a coder?
- [LapKeys](https://github.com/saddexed/LapKeys) - To all my gamers with no software support for brightness or refresh-rate out there
- [Prime Player Tweaks](https://github.com/saddexed/Prime-Player-Tweaks) - Functionality and CSS tweaks for the PrimeVideo player

----------------------------------------------------
*YOUR POWERS ARE MINE...* 
***FEARRR MAGNETO***  
Made by [saddexed](https://github.com/saddexed) 
